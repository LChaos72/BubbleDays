...
# This contains notes from the freecodecamp.org python tutorial
...
# strings
print("Giraffe\n Academy")  # next line
# \ is an escape sequence, any character after that will be rendered literally i.e here it wont be detected as quotation of print
...
# to insert kellu at index position 1 and other elements get pushed to the right
friends.insert(1, "kellu")
...
# ways to access-
print(months["Mar"])
# second value is some kind of default message which is displayed if key entered cant be found
print(months.get("Dec", "not found"))
...
# read files
# to open file; filename should have full path if not in same directory; second parameter is mode. "r" for reading(to read from the file and do stuff with the existing content), "w" for writing(can modify and write), "a" for appending(to add to end of file), "r+" for reading and writing
employee_file = open("employees.txt", "r")
# if file is readable or not (ie if in read mode it will return True)
...
# reads next line and prints out ; outputs with one line space
print(employee_file.readline())
print(employee_file.readlines())  # reads file and prints contents as a list
# outputs element at index 1 of the list created
...
# be careful, if run twice it will append again and not in new line! so add \n
employee_file.write("\nmoti")
employee_file.close()  # to close file
employee_file = open("employees.txt", "w")
# will overwrite on entire file! so file will end up having only this name as content
employee_file.write("hehe")
employee_file.close()
# here new file is created in the same directory and stuff is written to that
...
'''
	...
	# initialize function; we can basically map out what attributes the student should have
	def __init__(self, name, major, gpa, is_on_probation):

		# here, the name of the object that is passed gets stored under name attribute.
		self.name = name
		self.major = major
		self.gpa = gpa
...
lists - split()
# to display default value when dicname key doesnt exist
dictionaries - get(dicname, defaultvalue)
			   for word in words:  # here words is a list of words suppose and dic is dictionary
			       # here if word is there its value increments and if not then it is created and value set to 1, this is a good way to count instances and store at the same time
			       dic[word] = dic.get(word, 0)+1
tuples - if (a, b, c) > (b, d, c):
               # ans false cuz tuple compares first element if equal then second and so on. same with numbers. this is use in dictionaries because they cant have repetitive key names so making every key value pair a tuple and then sorting it will sort acc to tuple name (makes it more like a real dictionary)
               print True
